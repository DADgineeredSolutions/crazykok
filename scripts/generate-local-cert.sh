...
if command -v mkcert >/dev/null 2>&1; then
  mkcert -cert-file "$cert_file" -key-file "$key_file" \
    "$app_domain" "$app_domain_alias" "$api_domain" "$docs_domain" localhost 127.0.0.1 ::1
else
  ...
  openssl req -x509 -newkey rsa:2048 -sha256 -nodes -days 365 \
    ...
    -out "$cert_file" \
    -subj "/CN=$app_domain" \
    -addext "subjectAltName=DNS:$app_domain,DNS:$app_domain_alias,DNS:$api_domain,DNS:$docs_domain,DNS:localhost,IP:127.0.0.1,IP:::1"
fi
